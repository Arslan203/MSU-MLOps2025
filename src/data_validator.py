...
def validate_raw_data(
    recipes: pd.DataFrame, reviews: pd.DataFrame
) -> Tuple[bool, List[str]]:
    """
    ...
    missing_recipe_cols = [
        col for col in required_recipe_cols if col not in recipes.columns
    ]
    if missing_recipe_cols:
        errors.append(
            f"Отсутствуют необходимые столбцы в recipes: {missing_recipe_cols}"
        )

    # Проверка наличия необходимых столбцов в reviews
    required_review_cols = ["RecipeId", "Rating"]
    missing_review_cols = [
        col for col in required_review_cols if col not in reviews.columns
    ]
    if missing_review_cols:
        errors.append(
            f"Отсутствуют необходимые столбцы в reviews: {missing_review_cols}"
        )
        ...
        if "Rating" in reviews.columns:
            ...
            if not ratings.empty:
                if ratings.min() < 0 or ratings.max() > 5:
                    errors.append(
                        f"Rating должен быть в диапазоне [0, 5], получен [{ratings.min()}, {ratings.max()}]"
                    )
...
def validate_merged_data(
    data: pd.DataFrame, min_reviews_per_recipe: int
) -> Tuple[bool, List[str]]:
    """
    ...
    if "avg_rating" in data.columns:
        ...
        if not ratings.empty:
            if ratings.min() < 0 or ratings.max() > 5:
                errors.append(
                    f"avg_rating должен быть в диапазоне [0, 5], получен [{ratings.min()}, {ratings.max()}]"
                )
...
def validate_features(
    numerical_features: pd.DataFrame, text_features: pd.Series, target: pd.Series
) -> Tuple[bool, List[str]]:
    """
    ...
    missing_num_cols = [
        col for col in required_num_cols if col not in numerical_features.columns
    ]
    if missing_num_cols:
    ...
    if "minutes" in numerical_features.columns:
        ...
        if not minutes.empty:
            if minutes.min() < 0:
                errors.append(
                    f"minutes не может быть отрицательным, минимальное значение: {minutes.min()}"
                )
    ...
    if "n_steps" in numerical_features.columns:
        ...
        if not n_steps.empty:
            if n_steps.min() < 0:
                errors.append(
                    f"n_steps не может быть отрицательным, минимальное значение: {n_steps.min()}"
                )
    ...
    if "n_ingredients" in numerical_features.columns:
        ...
        if not n_ingredients.empty:
            if n_ingredients.min() < 0:
                errors.append(
                    f"n_ingredients не может быть отрицательным, минимальное значение: {n_ingredients.min()}"
                )
        ...
        if not pd.api.types.is_numeric_dtype(target):
            ...
            if not target_values.empty:
                if target_values.min() < 0 or target_values.max() > 5:
                    errors.append(
                        f"target должен быть в диапазоне [0, 5], получен [{target_values.min()}, {target_values.max()}]"
                    )

    # Проверка соответствия размеров
    if len(numerical_features) != len(text_features):
        errors.append(
            f"Размеры numerical_features ({len(numerical_features)}) и text_features ({len(text_features)}) не совпадают"
        )

    if len(numerical_features) != len(target):
        errors.append(
            f"Размеры numerical_features ({len(numerical_features)}) и target ({len(target)}) не совпадают"
        )
...
def validate_processed_features(
    ...
    if len(X_train_num) != len(X_train_text):
        errors.append(
            f"Размеры X_train_num ({len(X_train_num)}) и X_train_text ({len(X_train_text)}) не совпадают"
        )

    if len(X_val_num) != len(X_val_text):
        errors.append(
            f"Размеры X_val_num ({len(X_val_num)}) и X_val_text ({len(X_val_text)}) не совпадают"
        )

    if len(X_train_num) != len(y_train):
        errors.append(
            f"Размеры X_train_num ({len(X_train_num)}) и y_train ({len(y_train)}) не совпадают"
        )

    if len(X_val_num) != len(y_val):
        errors.append(
            f"Размеры X_val_num ({len(X_val_num)}) и y_val ({len(y_val)}) не совпадают"
        )
