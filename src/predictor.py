...
def postprocess_predictions(raw_predictions: Union[torch.Tensor, np.ndarray]) -> np.ndarray:
    """
...
def clip_predictions(predictions: np.ndarray, min_value: float = 0.0, max_value: float = 5.0) -> np.ndarray:
    """
...
def format_predictions_for_api(predictions: np.ndarray, 
                               recipe_ids: Optional[List] = None) -> List[Dict]:
    """
    ...
    for i, pred in enumerate(predictions):
        result = {
            'rating': float(pred),
            'rating_rounded': round(float(pred), 2)
        }
...
class RecipePredictor:
    ...
    def __init__(self, model, scaler_path: Optional[str] = None, 
    ...
    def predict(self, numerical_features: np.ndarray, 
    ...
    def predict_batch(self, numerical_features: np.ndarray,
                     text_features: Union[np.ndarray, List[str]],
                     recipe_ids: Optional[List] = None,
                     clip_values: bool = True,
                     min_rating: float = 0.0,
                     max_rating: float = 5.0) -> List[Dict]:
        """
...
def validate_predictions(predictions: np.ndarray, 
                         min_value: float = 0.0,
                         max_value: float = 5.0) -> Tuple[bool, List[str]]:
    """
    ...
    if predictions.size > 0:
        ...
        if pred_min < min_value:
            errors.append(f"predictions содержит значения меньше {min_value}: минимальное = {pred_min}")
        
        if pred_max > max_value:
            errors.append(f"predictions содержит значения больше {max_value}: максимальное = {pred_max}")
